...
class ParametersRegressor:
    ...
    def _init_pose_macro_local(
        ...
        for k, v in initial_local_changes_kwargs.items():
            if isinstance(v, torch.Tensor):
                assert v.shape[0] == batch_size
                local_changes_kwargs[k] = v.to(dtype=self.dtype, device=self.device)
            else:
                local_changes_kwargs[k] = torch.full((batch_size,), float(v), dtype=self.dtype, device=self.device)
    ...
    def _compute_macro_jacobian(
        ...
        J_local = self._compute_local_changes_jacobian(
            pose_parameters, phenotype_kwargs, local_changes_kwargs, idx
        )
        if J_local is not None:
            J_all = torch.cat([J_all, J_local], dim=-1)  # [B, V'*3, D_macro + D_local]

        return J_all
    
    def _compute_local_changes_jacobian(
        self,
        pose_parameters,
        phenotype_kwargs,
        ...
        """
        Jacobian of vertices wrt local changes (micro-shape parameters).
        Same finite difference strategy as phenotypes.
        """
        batch_size = pose_parameters.shape[0]
        L = len(local_changes_kwargs)

        if L == 0:
            return None

        # Expand everything
        pose_parameters_all = pose_parameters.unsqueeze(1).repeat(1, L+1, 1, 1, 1).flatten(0, 1)
        phenotype_kwargs_all = {k: v.unsqueeze(1).repeat(1, L+1).flatten(0, 1) for k, v in phenotype_kwargs.items()}
        local_changes_kwargs_all = None
        if local_changes_kwargs is not None:
            local_changes_kwargs_all = {k: v.unsqueeze(1).repeat(1, L+1).flatten(0, 1) for k, v in local_changes_kwargs.items()}
   
        # Add eps to each local-change parameters
        keys = list(local_changes_kwargs.keys())
        for i in range(1,len(local_changes_kwargs)+1):
            k = keys[i-1]
            indices = [i + j*(L+1) for j in range(batch_size)]
            local_changes_kwargs_all[k][indices] += self.eps
        ...
        verts_re = verts.reshape(batch_size, L+1, -1, 3)
        diffs = verts_re[:, 1:] - verts_re[:, [0]]
        
        diffs = diffs[:, :, idx].reshape(batch_size, L, -1)
        diffs = diffs.transpose(1, 2) / self.eps   # [B, V'*3, L]
    ...
    def __call__(
        ...
        initial_local_changes_kwargs: Optional[Dict[str, Any]] = None,
        ...
        initial_local_changes_kwargs = initial_local_changes_kwargs or {}
        pose_parameters, phenotype_kwargs, local_changes_kwargs = self._init_pose_macro_local(batch_size, initial_phenotype_kwargs, initial_pose_parameters, initial_local_changes_kwargs)
        ...
        for iter in range(max_n_iters):
            ...
            if optimize_phenotypes:
                # 1. Compute full Jacobian including local changes
                A_full = self._compute_macro_jacobian(pose_parameters, local_changes_kwargs, self.idx, phenotype_kwargs)
                b = (vertices_target[:, self.idx] - v_hat[:, self.idx]).reshape(batch_size, -1)

                n_pheno = len(optim_keys)
                n_local = len(local_changes_kwargs)
                n_total = n_pheno + n_local
                
                # Extract the corresponding phenotype weights
                pheno_weights = torch.tensor([
                    self.reg_weights[self.model.phenotype_labels.index(k)] for k in optim_keys
                ], dtype=self.dtype, device=self.device)
                
                # Define regularization weights for local changes (e.g. 1.0 or 2.0 to prevent overfitting)
                local_weights = torch.full((n_local,), 1.0, dtype=self.dtype, device=self.device)
                
                # Combine weights into a diagonal matrix for the total parameters
                W_reg = torch.diag(torch.cat([pheno_weights, local_weights]))
                
                # Augment A and b for Ridge Regression:
                # Minimize ||Ax - b||^2 + ||Wx||^2 by solving [A; W] x = [b; 0]
                A_aug = torch.cat([A_full, W_reg.unsqueeze(0).repeat(batch_size, 1, 1)], dim=1)
                b_aug = torch.cat([b, torch.zeros((batch_size, n_total), device=self.device)], dim=1)
                ...
                # 2. Split delta
                delta_pheno = delta[:, :n_pheno]  # [B, n_pheno]
                delta_local = delta[:, n_pheno:]  # [B, n_local]

                # 3. Apply updates
                for i, k in enumerate(optim_keys):
                    if shared_phenotypes:
                        shared_diff = torch.clamp(delta_pheno[:, i].mean(), -max_delta, max_delta)
                        phenotype_kwargs[k] = torch.clamp(phenotype_kwargs[k] + shared_diff, 0.01, 0.99).expand_as(phenotype_kwargs[k])
                    else:
                        diff = torch.clamp(delta_pheno[:, i], -max_delta, max_delta)
                        phenotype_kwargs[k] = torch.clamp(phenotype_kwargs[k] + diff, 0.01, 0.99)

                for i, k in enumerate(local_changes_kwargs):
                    if shared_phenotypes:
                        shared_diff = torch.clamp(delta_local[:, i].mean(), -max_delta, max_delta)
                        local_changes_kwargs[k] = torch.clamp(local_changes_kwargs[k] + shared_diff, -3.0, 3.0).expand_as(local_changes_kwargs[k])
                    else:
                        diff = torch.clamp(delta_local[:, i], -max_delta, max_delta)
                        local_changes_kwargs[k] = torch.clamp(local_changes_kwargs[k] + diff, -3.0, 3.0)
